...
df = pd.read_csv('lung.csv') # Load Data
...
corr = df.corr()['LUNG_CANCER'].abs() # Feature Selection (keep only useful features)
features = corr[corr>0.1].index.drop('LUNG_CANCER')
...
joblib.dump(model, 'lung_model.pkl')  # Save the model as .pkl automatically
print("lung_model.pkl file created successfully!")
